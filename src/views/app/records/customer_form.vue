<template>
  <div>
    ...
    <b-form>
      <b-row>
        ...
        <span>{{ $t('forms.customer.first_name') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.first_name.$model" :state="!$v.form.first_name.$error"></b-form-input>
              <span> {{ $t('forms.customer.first_name') }}</span>
        ...
        <span>{{ $t('forms.customer.last_name') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.last_name.$model" :state="!$v.form.last_name.$error"></b-form-input>
              <span> {{ $t('forms.customer.last_name') }}</span>
        ...
        <span>{{ $t('forms.customer.email') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.email.$model" :state="!$v.form.email.$error"></b-form-input>
              <span> {{ $t('forms.customer.email') }}</span>
      ...
      <b-form-input v-model="form.phone_number" ></b-form-input>
		<span>{{ $t('forms.customer.phone_number') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.phone_number.$model" :state="!$v.form.phone_number.$error"></b-form-input>
              <span> {{ $t('forms.customer.phone_number') }}</span>
      ...
      <b-form-input v-model="form.address" ></b-form-input>
		<span>{{ $t('forms.customer.address') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.address.$model" :state="!$v.form.address.$error"></b-form-input>
              <span> {{ $t('forms.customer.address') }}</span>
    ...
    <b-form-invalid-feedback>Purchase date is required!</b-form-invalid-feedback>
          ...
          <div class="mx-2">
            <b-form class="av-tooltip tooltip-label-right">
              <datepicker :default-value="today" type="date" v-model="form.dob" :placeholder="$t('forms.customer.dob')"
                value-type="format" format="DD-MM-YYYY" style="width: 100% !important;"></datepicker>
            </b-form>
          </div>
        </b-colxx>
        <b-colxx xxs="12" xs="3" lg="3" class="mb-3">
		...
		<span>{{ $t('forms.customer.gender') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            ...
            :state="!$v.form.gender.$error"></b-form-input> -->
              ...
              <b-form-input style="display:none" type="text" v-model.trim="$v.form.gender.$model"
                :state="!$v.form.gender.$error" />

              <v-select v-model="$v.form.gender.$model" :reduce="gender => gender.id" label="name"
                aria-placeholder="Select Gender"
                :options="[{ id: 1, name: 'Male' }, { id: 2, name: 'Female' }]"></v-select>
      ...
      <b-form-input v-model="form.occupation" ></b-form-input>
		<span>{{ $t('forms.customer.occupation') }}</span>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <label class="form-group has-top-label">
              <b-form-input v-model="$v.form.occupation.$model" :state="!$v.form.occupation.$error"></b-form-input>
              <span> {{ $t('forms.customer.occupation') }}</span>
      ...
      <b-file v-model="form.driver_licence_front_picture" ></b-file> -->
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <b-form-group :label="$t('forms.customer.driver_licence_front_picture')">
              <b-form-input style="display:none" type="text" v-model.trim="$v.form.driver_licence_front_picture.$model"
                :state="!$v.form.driver_licence_front_picture.$error" />
              <!-- @change="onFileChange" -->

              <b-form-file v-model.trim="$v.form.driver_licence_front_picture.$model"
                drop-placeholder="Drop file here..." accept="image/*">
              </b-form-file>
              <b-form-invalid-feedback v-if="$v.form.driver_licence_front_picture.$error"> Picture is
      ...
      <b-file v-model="form.driver_licence_back_picture" ></b-file> -->
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <b-form-group :label="$t('forms.customer.driver_licence_back_picture')">
              <b-form-input style="display:none" type="text" v-model.trim="$v.form.driver_licence_back_picture.$model"
                :state="!$v.form.driver_licence_back_picture.$error" />
              <!-- @change="onFileChange" -->

              <b-form-file v-model.trim="$v.form.driver_licence_back_picture.$model"
                drop-placeholder="Drop file here..." accept="image/*">
              </b-form-file>
              <b-form-invalid-feedback v-if="$v.form.driver_licence_back_picture.$error"> Picture is
                required!</b-form-invalid-feedback>
            </b-form-group>
          </b-form>
    ...
    <b-form-invalid-feedback>Driver licence expiry!</b-form-invalid-feedback>
          ...
          <b-form class="av-tooltip tooltip-label-right">
            <datepicker :default-value="today" type="date" v-model="form.driver_licence_expiry"
              :placeholder="$t('forms.customer.driver_licence_expiry')" value-type="format" format="DD-MM-YYYY"
              style="width: 100% !important;"></datepicker>
          </b-form>
        ...
        <b-colxx xxs="12" xs="3" lg="3" class="mb-3">
          <!-- <label class="form-group has-top-label">
            ...
            <datepicker :bootstrap-styling="true" :disabled-dates="{ to: new Date() }"
              v-model="form.secondary_id_expiry" format="dd-MM-yyyy" style="width: 100% !important;"></datepicker>
            <span>{{ $t('forms.customer.secondary_id_expiry') }}</span>
            <b-form-invalid-feedback>Secondary id expiry!</b-form-invalid-feedback>
          </label> -->
          <b-form class="av-tooltip tooltip-label-right">
            <b-form-input style="display:none" type="text" v-model.trim="form.secondary_id_expiry" />

            <datepicker :default-value="today" type="date" v-model="form.secondary_id_expiry"
              :placeholder="$t('forms.customer.secondary_id_expiry')" value-type="format" format="DD-MM-YYYY"
              style="width: 100% !important;"></datepicker>
          </b-form>
        </b-colxx>
        <b-colxx xxs="12" xs="3" lg="3" class="mb-3">
      ...
      <b-button v-if="!editing_mode" @click.stop="save_customer_record" variant="primary" class="mt-4 mb-4">{{ 'Save'
        }}</b-button>
      <div v-else>
        <b-button @click.stop="update_customer_record(customer_id)" variant="secondary" class="mt-4 mb-4">{{ ' Update'
          }}</b-button>
